...
function isAfreecaDonation(data: unknown): data is {
  type: string;
  nickname: string;
  message: string;
  payAmount: number;
 ...
 */
export const biancaDonationListener = onRequest(
  ...
  async (req, res) => {
    ...
    const streamers: StreamerDoc[] = streamersSnap.docs
      .map((doc) => ({ id: doc.id, ...doc.data() } as StreamerDoc))
    ...
    for (const streamer of streamers) {
      ...
      ws.on("message", async (raw: Buffer) => {
        try {
          ...
          if (!isAfreecaDonation(data)) return;
